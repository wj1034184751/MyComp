...
{
    [AbpAuthorize]
    public class STLedScreenAppService : AsynPermissionAppService<STLedScreen, STLedScreenDto, string, PagedSTLedScreenRequestDto, STLedScreenDto, STLedScreenDto>, ISTLedScreenAppService
    {
        private readonly IRepository<AbpAttachment, string> _repositoryAttachment;
        public STLedScreenAppService(IRepository<STLedScreen, string> repository, IRepository<AbpAttachment, string> repositoryAttachment) : base(repository)
        {
            _repositoryAttachment = repositoryAttachment;
        }

        protected override IQueryable<STLedScreen> CreateFilteredQuery(PagedSTLedScreenRequestDto input)
        {
        ...
        [ActionName("GetPage")]
        public override async Task<PagedResultDto<STLedScreenDto>> GetAllAsync(PagedSTLedScreenRequestDto input)
        {
        ...
        public override async Task<STLedScreenDto> CreateAsync(STLedScreenDto input)
        {
            try
            {
                var model = ObjectMapper.Map<STLedScreen>(input);
        ...
        public override async Task<STLedScreenDto> UpdateAsync(STLedScreenDto input)
        {
            try
            {
