...
{
    [AbpAuthorize]
    public class STOximeterAppService : AsynPermissionAppService<STOximeter, STOximeterDto, string, PagedSTOximeterRequestDto, STOximeterDto, STOximeterDto>, ISTOximeterAppService
    {
        private readonly IRepository<AbpAttachment, string> _repositoryAttachment;
        public STOximeterAppService(IRepository<STOximeter, string> repository, IRepository<AbpAttachment, string> repositoryAttachment) : base(repository)
        {
            _repositoryAttachment = repositoryAttachment;
        }

        protected override IQueryable<STOximeter> CreateFilteredQuery(PagedSTOximeterRequestDto input)
        {
        ...
        [ActionName("GetPage")]
        public override async Task<PagedResultDto<STOximeterDto>> GetAllAsync(PagedSTOximeterRequestDto input)
        {
        ...
        public override async Task<STOximeterDto> CreateAsync(STOximeterDto input)
        {
            try
            {
                var model = ObjectMapper.Map<STOximeter>(input);
        ...
        public override async Task<STOximeterDto> UpdateAsync(STOximeterDto input)
        {
            try
            {
