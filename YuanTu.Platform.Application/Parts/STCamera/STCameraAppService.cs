...
{
    [AbpAuthorize]
    public class STCameraAppService : AsynPermissionAppService<STCamera, STCameraDto, string, PagedSTCameraRequestDto, STCameraDto, STCameraDto>, ISTCameraAppService
    {
        private readonly IRepository<AbpAttachment, string> _repositoryAttachment;
        public STCameraAppService(IRepository<STCamera, string> repository, IRepository<AbpAttachment, string> repositoryAttachment) : base(repository)
        {
            _repositoryAttachment = repositoryAttachment;
        }

        protected override IQueryable<STCamera> CreateFilteredQuery(PagedSTCameraRequestDto input)
        {
        ...
        [ActionName("GetPage")]
        public override async Task<PagedResultDto<STCameraDto>> GetAllAsync(PagedSTCameraRequestDto input)
        {
        ...
        public override async Task<STCameraDto> CreateAsync(STCameraDto input)
        {
            try
            {
                var model = ObjectMapper.Map<STCamera>(input);
        ...
        public override async Task<STCameraDto> UpdateAsync(STCameraDto input)
        {
            try
            {
