...
{
    [AbpAuthorize]
    public class STOtherAppService : AsynPermissionAppService<STOther, STOtherDto, string, PagedSTOtherRequestDto, STOtherDto, STOtherDto>, ISTOtherAppService
    {
        private readonly IRepository<AbpAttachment, string> _repositoryAttachment;
        public STOtherAppService(IRepository<STOther, string> repository, IRepository<AbpAttachment, string> repositoryAttachment) : base(repository)
        {
            _repositoryAttachment = repositoryAttachment;
        }

        protected override IQueryable<STOther> CreateFilteredQuery(PagedSTOtherRequestDto input)
        {
        ...
        [ActionName("GetPage")]
        public override async Task<PagedResultDto<STOtherDto>> GetAllAsync(PagedSTOtherRequestDto input)
        {
        ...
        public override async Task<STOtherDto> CreateAsync(STOtherDto input)
        {
            try
            {
                var model = ObjectMapper.Map<STOther>(input);
        ...
        public override async Task<STOtherDto> UpdateAsync(STOtherDto input)
        {
            try
            {
